...
var timer, previous, current;
var delta = 2,
    topMargin = 30;
function encrypt() {
  if (timer == null) {
  timer = setInterval(function() {
    $('.upper').height($('.upper').height() - delta);
    $('.lower').height($('.lower').height() + delta);
    
    layoutRegions();
    current = $('.highlighter').text();
    
    if ($('.upper').height() < topMargin) {
      cancelTimer();
    }
    
    var paddingTop = +$('.lower')
        .css('paddingTop')
        .replace('px', '');
    ...
    $('.lower').css('paddingTop', paddingTop + 'px');
  }, 55);
  }
}
...
$(document).ready(function() {
  previous = current = $('.highlighter').text();
  $('#type').on('click', function() {encrypt();});
});
